...
std::string extractPriorMonthYear(const std::string& input) {
    ...
    return monthNames[month - 1] + " " + std::to_string(year);
}

std::string reformatDateAs_MMDDYYYY(const std::string& input) {
...
std::string getReportDateAs_MMYYYY(const std::string& input) {
    if (input.size() != 10 || input[4] != '/' || input[7] != '/') {
        return "Invalid format";
    }
    ...
    (monthInt == 1) ? monthInt = 12 : monthInt -= 1;
    std::string monthStr = std::to_string(monthInt);

    if (monthStr.size() == 1)
        monthStr = "0" + monthStr;

    return monthStr + "/" + "01" + "/" + year;
}
